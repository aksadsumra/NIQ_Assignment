...
class CountDetectedObjects:
    ...
    def execute(self, image, threshold) -> CountResponse:
        ...
        predictions = self.__find_valid_predictions(
            image,
            threshold
        )
        ...
        self.__object_count_repo.update_values(
            object_counts
        )
    ...
    def __find_valid_predictions(
            self,
            image,
            threshold):

        predictions = self.__object_detector.predict(
            image
        )
